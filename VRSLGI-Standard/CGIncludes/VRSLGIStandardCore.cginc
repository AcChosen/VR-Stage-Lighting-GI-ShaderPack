...
#include "VRSLGIStandardInput.cginc"
...
inline FragmentCommonData MetallicSetup (float4 i_tex)
{
    FragmentCommonData o = (FragmentCommonData)0;
        ...
        half4 mainTex = float4(1,1,1,1);
            ...
            mainTex = MainTexture(i_tex.xy);
        #endif
        half2 metallicGloss = MetallicGlossStandard(mgm, mainTex);
        o.metallicGlossMap = mgm;
    #else
...
struct VertexOutputForwardBase
{
    ...
    float3 posWorld                     : TEXCOORD8;
        ...
        #endif
    #endif
    #if defined(_AREALIT_ON) || defined(LTCGI)
        float4 areaLitLTCGIuv                   :TEXCOORD11;
    #endif
...
VertexOutputForwardBase vertForwardBase (VertexInput v)
{
    ...
    //VRSL Stuff
       ...
       o.shadowMaskUV = VRSLShadowMaskCoords(v);
       #else
            #ifdef _VRSL_GI_ENFORCELIMIT
                o.meshWorldPos = mul(unity_ObjectToWorld, float4(0,0,0,1));
                o.QuadLightPosition[0] = float4(0,0,0,0);
                o.QuadLightPosition[1] = float4(0,0,0,0);
                o.QuadLightPosition[2] = float4(0,0,0,0);
                o.QuadLightPosition[3] = float4(0,0,0,0);
                o.QuadLightColor[0] = float4(0,0,0,0);
                ...
                o.QuadLightPosition[2] = qLightPositions[2];
                o.QuadLightPosition[3] = qLightPositions[3];
                o.QuadLightColor[0] = qLightColors[0];
    ...
    #endif
    #if _AREALIT_ON
        o.areaLitLTCGIuv.xy = AreaLitCoords(v);
    #endif
    #ifdef LTCGI
        o.areaLitLTCGIuv.zw = LTCGICoords(v);
    #endif
    //End VRSL Stuff
...
half4 fragForwardBaseProj (VertexOutputForwardBase i, bool frontFace)
{
	...
	// c.rgb += (s.diffColor + s.specColor);
            ...
            ld.positions[0] = i.QuadLightPosition[0];
            ld.positions[1] = i.QuadLightPosition[1];
            ld.positions[2] = i.QuadLightPosition[2];
            ld.positions[3] = i.QuadLightPosition[3];
            ld.colors[0] = i.QuadLightColor[0];
            ...
            -s.eyeVec, 
            _VRSLGlossiness, 
            s.diffColor, i.finalLightCount) *_VRSLProjectorStrength;
        #else
            c.rgb += VRSLGI(s.posWorld, s.normalWorld, _VRSLGlossiness, -s.eyeVec, s.diffColor, float2(1,1), i.tex.xy, 1.0) *_VRSLProjectorStrength;
        #endif
    #endif
        ...
        c.rgb += areaLitColor * _AreaLitStrength * 1000;
    #endif
    return c - saturate(min(min(c.r, c.g), min(c.b, c.a)));
}
#endif
...
half4 fragForwardBaseInternal (VertexOutputForwardBase i, bool frontFace)
{
    ...
    half4 c = UNITY_BRDF_PBS (s.diffColor, s.specColor, s.oneMinusReflectivity, s.smoothness, s.normalWorld, -s.eyeVec, gi.light, gi.indirect);
        ...
        LTCGI_Contribution(
            s.posWorld,
            normalize(s.normalWorld.xyz),
            normalize(camera_pos - s.posWorld),
            roughness,
            i.areaLitLTCGIuv.zw,
            diff,
            spec
        );
        c.rgb *= ((saturate(diff + 0.1)) * _LTCGIStrength);
        c.rgb += (spec * _LTCGIStrength);
    #endif



    #if _AREALIT_ON
        float4 alOcclusion = AreaLitOcclussionMask(i.areaLitLTCGIuv.xy);
        AreaLightFragInput ai;
        ai.pos = s.posWorld;
        ai.normal = s.normalWorld;
        ai.view = -s.eyeVec;
        ai.roughness = roughness * _AreaLitRoughnessMult;
        ai.occlusion = float4(occlusion,occlusion,occlusion, 1) * alOcclusion;
        ai.screenPos = i.pos.xy;
        half4 diffTerm, specTerm;
        ShadeAreaLights(ai, diffTerm, specTerm, true, !IsSpecularOff(), IsStereo());
    #endif
    ...
    c.rgb += Emission(i.tex.xy);
    #if VRSL_ENABLED
        float3 emissionMap = SampleTexture(_DMXEmissionMap, i.tex.xy);
        #if _VRSL_MIX_MULT
